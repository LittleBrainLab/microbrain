...
def dcm2nii(dataDir, outDir, rawDir, thisSub, mag_image=False, phase_image=False):
    ...
    if not os.path.exists(convertDir):
        subprocess.run(['mkdir', convertDir], stdout=subprocess.PIPE, universal_newlines=True)
    ...
    if not os.path.exists(niiOutFile):
        ...
        if os.path.exists(dataDir):
            if is_tool('dcm2niix'):
                ...
                print("MicroBrain Setup: Could not find dcm2niix, make sure it is installed to your path")
                stdout = ''
                return_code = 1
        else:
...
def main(argv):
    ...
    try:
        ...
        opts, args = getopt.getopt(argv,"hs:i:",["idcm=","idcm_reversePE=","idcm_fieldmap=","idcm_fieldmap_smooth="])
    ...
    if not os.path.exists(subDir):
        process = subprocess.run(['mkdir', subDir], stdout=subprocess.PIPE, universal_newlines=True)
    
    origDir = outputDir + '/' + subID + '/orig/'
    if not os.path.exists(origDir):
        process = subprocess.run(['mkdir', origDir], stdout=subprocess.PIPE, universal_newlines=True)
    ...
    if topup:
        ...
        [fdwi, fbval, fbvec, fjson, stdout, returncode] = dcm2nii(inputDir, outputDir, 'orig_normalPE/', subID)
        if returncode != 0:
            print('DSurfer: dcm2niix returned an error, make sure it is installed correctly and that dicom files exist')
            sys.exit()

        [fdwi_reversePE, fbval_reversePE, fbvec_reversePE, fjson_reversePE, stdout, returncode] = dcm2nii(inputDir_reversePE, outputDir, 'orig_reversePE/', subID)
        if returncode != 0:
            print('DSurfer: dcm2niix returned an error, make sure it is installed correctly and that dicom files exist')
            sys.exit()
        ...
        process = subprocess.run(['fslmerge', '-t', fdwi_merge, fdwi, fdwi_reversePE], stdout=subprocess.PIPE, universal_newlines=True)
        ...
        bvals_reversePE, bvecs_reversePE = read_bvals_bvecs(fbval_reversePE, fbvec_reversePE)
        ...
        np.savetxt(fbval_merge, bvals_merge[None,:], delimiter=' ', fmt='%01d')
        np.savetxt(fbvec_merge, bvecs_merge, delimiter=' ', fmt='%05f')
        np.savetxt(fpe_ind, pe_direction_ind[None,:], delimiter=' ', fmt='%01d')
        if os.path.exists(fjson):
            process = subprocess.run(['cp', fjson, fjson_merge], stdout=subprocess.PIPE, universal_newlines=True)
        
    elif fieldmap:
        # Given an input folder, convert to NIFTI and output into appropriate folder.
        # Function will skip this step if orig folder already exists with converted NIFTI File
        [fdwi, fbval, fbvec, fjson, stdout, returncode] = dcm2nii(inputDir, outputDir, 'orig/', subID)
        if returncode != 0:
            print('DSurfer: dcm2niix returned an error, make sure it is installed correctly and that dicom files exist')
            sys.exit()

        # Convert magnitude image
        [ffieldmap_mag1, fnotbval, fnotbvec, ffieldmap_mag_json, stdout, returncode] = dcm2nii(fieldmapDir_mag, outputDir, 'orig_fieldmap/', subID, True, False)
        if returncode != 0:
            print('DSurfer: dcm2niix returned an error, make sure it is installed correctly and that dicom files exist')
            sys.exit()

        # Convert phase image
        [ffieldmap_phase, fnotbval, fnotbvec, ffieldmap_phase_json, stdout, returncode] = dcm2nii(fieldmapDir_phase, outputDir, 'orig_fieldmap/', subID, False, True)
        if returncode != 0:
            print('DSurfer: dcm2niix returned an error, make sure it is installed correctly and that dicom files exist')
            sys.exit()
        ...
        ffieldmap_mag2 = ffieldmap_mag1.replace('e1' + fsl_ext(), 'e2' + fsl_ext())
        ffieldmap_mag = ffieldmap_mag1.replace('e1' + fsl_ext(), 'emean' + fsl_ext())
        process = subprocess.run(['fslmaths',ffieldmap_mag1,'-add',ffieldmap_mag2,'-div','2',ffieldmap_mag], stdout=subprocess.PIPE, universal_newlines=True)

        ffieldmap_mag_brain1 = ffieldmap_mag.replace(fsl_ext(), '_brain1')
        process = subprocess.run(['bet', ffieldmap_mag,ffieldmap_mag_brain1,'-m', '-B'], stdout=subprocess.PIPE, universal_newlines=True)
        #ffieldmap_mag_brain = ffieldmap_mag_brain + fsl_ext()

        ffieldmap_mag_brain1_mask = ffieldmap_mag_brain1 + '_mask' + fsl_ext()
        ffieldmap_mag_brain_mask = ffieldmap_mag_brain1_mask.replace('brain1_mask' + fsl_ext(), 'brain_mask' + fsl_ext())
        process = subprocess.run(['fslmaths',ffieldmap_mag_brain1_mask,'-kernel','box','20','-dilF',ffieldmap_mag_brain_mask], stdout=subprocess.PIPE, universal_newlines=True)
        ...
        ffieldmap_mag_brain = ffieldmap_mag_brain_mask.replace('_mask' + fsl_ext(), fsl_ext())
        process = subprocess.run(['fslmaths',ffieldmap_mag,'-mul',ffieldmap_mag_brain_mask, ffieldmap_mag_brain], stdout=subprocess.PIPE, universal_newlines=True)

        ffieldmap_rad = ffieldmap_mag.replace('emean'+ fsl_ext(), 'fieldmap_rad' + fsl_ext())
        process = subprocess.run(['fsl_prepare_fieldmap', 'SIEMENS', ffieldmap_phase, ffieldmap_mag_brain, ffieldmap_rad, fieldmap_echo_diff], stdout=subprocess.PIPE, universal_newlines=True)
    
        if smooth_radius != 0: 
            process = subprocess.run(['fugue', '--loadfmap=' + ffieldmap_rad, '-s', str(smooth_radius), '--savefmap=' + ffieldmap_rad], stdout=subprocess.PIPE, universal_newlines=True)
            
        ffieldmap_hz = ffieldmap_rad.replace('rad' + fsl_ext(), 'hz' + fsl_ext())
        process = subprocess.run(['fslmaths', ffieldmap_rad, '-div', '6.28318531', ffieldmap_hz], stdout=subprocess.PIPE, universal_newlines=True)
    else:
        # Given an input folder, convert to NIFTI and output into appropriate folder.
        # Function will skip this step if orig folder already exists with converted NIFTI File
        [fdwi, fbval, fbvec, fjson, stdout, returncode] = dcm2nii(inputDir, outputDir, 'orig/', subID)
