...
def denoiseNLSAM(fdwi, fmask, fbval, fbvec, denoiseDir, patch_radius = 2, cpu_num = 0):
    ...
    fdwi_bname = os.path.basename(fdwi)
    fdenoise = denoiseDir + fdwi_bname.replace(fsl_ext(), '_DNLSAM' + fsl_ext())
    ...
    if not os.path.exists(fdenoise):
        if is_tool('nlsam_denoising'):
            ...
            print("Microbrain preproc: Could not find nlsam_denoising, make sure nlsam is installed to your path")
            stdout = ''
            return_code = 1
    else:
...
def stabilizeNLSAM(fdwi, fmask, fbval, fbvec, denoiseDir, patch_radius =2):
    ...
    fdwi_bname = os.path.basename(fdwi)
    fdenoise = denoiseDir + fdwi_bname.replace(fsl_ext(), '_NOTEXIST' + fsl_ext())
    fsigma = denoiseDir + fdwi_bname.replace(fsl_ext(), '_sigma' + fsl_ext())
    fncoil = denoiseDir + fdwi_bname.replace(fsl_ext(), '_Ncoil' + fsl_ext())
    fstab = denoiseDir + fdwi_bname.replace(fsl_ext(), '_STAB' + fsl_ext())
...
def denoiseMPPCA(fdwi, fbval, fbvec, denoiseDir, patch_radius =2):
    ...
    fdwi_bname = os.path.basename(fdwi)
    fdenoise = denoiseDir + fdwi_bname.replace(fsl_ext(), '_DMPPCA' + fsl_ext())
...
def surface_based_brain_mask_onMD(finmask, fmd, maskDir):
    ...
    if not os.path.exists(maskDir):
        subprocess.run(['mkdir', maskDir], stdout=subprocess.PIPE, universal_newlines=True)
    ...
    fmask_insurf = maskDir + fmd_bname.replace('MD' + fsl_ext(), 'init_mask.vtk')
    if not os.path.exists(fmask_insurf):
        os.system('mirtk extract-surface ' + finmask_invert + ' ' + fmask_insurf + ' -isovalue 0.5 -close')

    # Deform mesh around MD to remove a bunch of non brain voxels
    fmask_outsurf = maskDir + fmd_bname.replace('MD' + fsl_ext(), 'deform_mask.vtk')
    ...
    fmask_outsurf2 = maskDir + fmd_bname.replace('MD' + fsl_ext(), 'deform_mask2.vtk')
    ...
    fmask_out = maskDir + fmd_bname.replace('MD' + fsl_ext(), 'micromask' + fsl_ext()) 
...
def fslv6p0_topup(fb0s, facq):
    ...
    if not os.path.exists(ftopup_unwarped + fsl_ext()):
        if is_tool('topup'):
            process = subprocess.run(['topup', '--imain=' + fb0s, '--datain=' + facq, '--config=b02b0.cnf', '--out=' + ftopup, '--fout=' + ftopup_field, '--iout=' + ftopup_unwarped,'-v'],stdout=subprocess.PIPE, universal_newlines=True)
            stdout = process.stdout
            return_code = process.returncode
        else:
            print("MicroBrain Preproc: Could not find topup, make sure it is installed to your path")
            stdout = ''
            return_code = 1
    else:
