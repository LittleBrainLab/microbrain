...
def get_medial_surf_from_wm_gm(wmSurf, pialSurf):
    ...
    pialPntMap = vtk_to_numpy(
        pialSurf.GetPointData().GetArray("InitialPoints"))
    ...
    for pointID in pointList:
        if pointID % 1000 == 0:
            output_stream.write(
                "Calculating Medial Surface: Vert " + str(pointID) + "    \r")
            output_stream.flush()

        sys.stdout.flush()
        thisGMpnt = np.mean(
            pialPnts[np.where(pialPntMap == pointID), :], axis=0)
        ...
        if np.isnan(thisGMpnt).any() or thisGMpnt.shape[0] == 0:
            ...
            allGMpnts = pialPnts[np.squeeze(
                np.where(pialPntMap == pointID)), :]
            if allGMpnts.shape == (3,):
                allGMpnts = allGMpnts[None, :]
            allWMpnts = np.repeat(
                wmPnts[pointID, :][None, :], allGMpnts.shape[0], axis=0)
            distance = np.sqrt(
                np.sum(np.square(allWMpnts - allGMpnts), axis=1))

            minPnt = allGMpnts[distance == np.min(distance), :]
            if minPnt.shape[0] == 2:
                minPnt = np.squeeze(minPnt[0, 0:3])
            medialPnts[pointID, :] = wmPnts[pointID, :] + \
                (minPnt - wmPnts[pointID, :])*0.5
...
def generate_surface_from_wm(wm_img):
    ...
    surf_verts, surf_faces, surf_normals, values = measure.marching_cubes_lewiner(
        wm_data, spacing=wm_hdr.get_zooms(), allow_degenerate=False)
    ...
    for i in range(0, surf_verts.shape[0]):
        points.InsertNextPoint(
            surf_verts[i, 0], surf_verts[i, 1], surf_verts[i, 2])
...
def generate_surface_from_wm_2surf(wm_img):
    ...
    lh_surf = {}
    lh_surf_verts, lh_surf_faces, lh_surf_normals, values = measure.marching_cubes_lewiner(
        lh_wm, spacing=wm_hdr.get_zooms(), allow_degenerate=False)
    ...
    rh_surf = {}
    rh_surf_verts, rh_surf_faces, rh_surf_normals, values = measure.marching_cubes_lewiner(
        rh_wm, spacing=wm_hdr.get_zooms(), allow_degenerate=False)
    ...
    pixdims = wm_hdr.get_zooms()
    lh_surf_verts[:, 0] = np.array(
        [vert[0]/pixdims[0] for vert in lh_surf_verts])
    lh_surf_verts[:, 1] = np.array(
        [vert[1]/pixdims[1] for vert in lh_surf_verts])
    lh_surf_verts[:, 2] = np.array(
        [vert[2]/pixdims[2] for vert in lh_surf_verts])

    rh_surf_verts[:, 0] = np.array(
        [vert[0]/pixdims[0] for vert in rh_surf_verts])
    rh_surf_verts[:, 1] = np.array(
        [vert[1]/pixdims[1] for vert in rh_surf_verts])
    rh_surf_verts[:, 2] = np.array(
        [vert[2]/pixdims[2] for vert in rh_surf_verts])
    ...
    tmp_lh_verts = np.concatenate(
        (tmp_lh_verts, np.ones([1, tmp_lh_verts.shape[1]])))
    ...
    tmp_rh_verts = np.concatenate(
        (tmp_rh_verts, np.ones([1, tmp_rh_verts.shape[1]])))
    ...
    for i in range(0, lh_surf_verts.shape[0]):
        lh_points.InsertNextPoint(
            lh_surf_verts[i, 0], lh_surf_verts[i, 1], lh_surf_verts[i, 2])
    ...
    for i in range(0, rh_surf_verts.shape[0]):
        rh_points.InsertNextPoint(
            rh_surf_verts[i, 0], rh_surf_verts[i, 1], rh_surf_verts[i, 2])
...
def vertex_proximity(point_array, k, locator):
    ...
    dmin_to_other_vert = scipy.spatial.distance.cdist(
        thisPnt[:, None].T, closestPnt[:, None].T)
...
def cell_intersection(k, point_array, dir_array, cell_locator):
    tol0 = 0.00001
    thisPnt = point_array[k, :] + dir_array[k, :] / \
        np.linalg.norm(dir_array[k, :], axis=0)*tol0
    # thisPnt = point_array[k,:]

    movedPnt = point_array[k, :] + dir_array[k, :] + \
        dir_array[k, :]/np.linalg.norm(dir_array[k, :], axis=0)*0.01
    # movedPnt = point_array[k,:] + dir_array[k,:]
    ...
    cell_locator.IntersectWithLine(
        thisPnt.tolist(), movedPnt.tolist(), tol0, t, pos, pcoords, subId)
    ...
    if t <= tol0 or t > 1.0 - tol0:
        ...
        for no_intersection in np.arange(1, 0, -0.05):
            ...
            # Worst case scenario two vertices move towards each other at this distance, divide by 2 to avoid intersection
            no_intersection = no_intersection / 2.1
            if t <= tol0 or t > 1.0 - tol0:
...
def get_medial_surface(surfVTK1, surfVTK2):
    ...
    normal_array = vtk_to_numpy(
        surfVTK_norm.GetPointData().GetArray("Normals"))
    ...
    for i in range(0, surf1_points.shape[0]):
        ...
        pcoords = [0.0, 0.0, 0.0]
        subId = vtk.mutable(0)
        pntLocator.IntersectWithLine(
            thisPnt.tolist(), movedPnt.tolist(), tol0, tol1, pos, pcoords, subId)
        closestPnt = pos

        if closestPnt == [0, 0, 0]:
            cellId = vtk.mutable(0)
            closestPointDist2 = vtk.mutable(0)
            pntLocator.FindClosestPoint(
                thisPnt.tolist(), closestPnt, cellId, subId, closestPointDist2)
...
def get_radial_surface(medialVTK, normalSurfVTK, vecImgVTK, sformMat):
    ...
    vecVTK = interpolate_voldata_to_surface(
        medialVTK, vecImgVTK, sformMat, pntDataName='EVECS')
...
def correct_surface_by_normal(surfVTK, targetImageVTK, gradImageVTK, sformMat, target_val, interMaskVTK,
    ...
    for i in range(0, n_iter):
        for m in range(0, surfNum):
            ...
            curv_min = vtk_to_numpy(
                curvSurf.GetPointData().GetArray("Minimum_Curvature"))
            curver.SetCurvatureType(2)  # Minimum 3, Maximum 2, Mean 1, Gauss 0
            curver.Update()
            curvSurf = curver.GetOutput()
            curv_max = vtk_to_numpy(
                curvSurf.GetPointData().GetArray("Maximum_Curvature"))
            curvVals = (0.25*curv_max + curv_min)/2
            print("Min Max Avg Curv", np.max(curvVals),
                  np.min(curvVals), np.mean(curvVals))
            del curver
            ...
            num_points = np.size(vtk_to_numpy(
                thisSurfVTK.GetPoints().GetData()), axis=0)
            ...
            surfVTK_interp = interpolate_voldata_to_surface(
                thisSurfVTK, targetImageVTK, sformMat)
            intensity_array = vtk_to_numpy(
                surfVTK_interp.GetPointData().GetArray("NIFTI"))

            # Interpolate gradient onto surface
            surfVTK_grad = interpolate_voldata_to_surface(
                thisSurfVTK, gradImageVTK, sformMat)
            gradient_array = vtk_to_numpy(
                surfVTK_grad.GetPointData().GetArray("NIFTI"))
            ...
            interMaskSurf = interpolate_voldata_to_surface(
                thisSurfVTK, interMaskVTK, sformMat)
            surfMask = vtk_to_numpy(
                interMaskSurf.GetPointData().GetArray("NIFTI"))
            ...
            if limitImg:
                limitSurf = interpolate_voldata_to_surface(
                    thisSurfVTK, limitImg, sformMat)
                limit_vals = vtk_to_numpy(
                    limitSurf.GetPointData().GetArray("NIFTI"))
                move_limits[limit_vals < limit] = 0

            # show_normals(point_array, normal_array)
            e0_array = np.concatenate((np.expand_dims(-normal_array[:, 2], axis=1), np.zeros(
                (normal_array.shape[0], 1)), np.expand_dims(normal_array[:, 0], axis=1)), axis=1)
            ...
            if reverse_direction:
                # Use this one for FA (If using B0 gradient)
                intensity_diff = (intensity_array -
                                  target_intensity)[:, None] / intensity_scale
            else:
                # Use this one for MD (If using B0 gradient)
                intensity_diff = (target_intensity -
                                  intensity_array)[:, None] / intensity_scale

            # For ISMRM 2019 moved vertices in direction of normal
            direction_array = normal_array
            ...
            dir_array = surfMask[:, None] * \
                (term1 + lambda_S * -normal_array * curvVals[:, None])
           ...
           #    neighbor_ind = neighbors[k]
            ...
            move_vert = surfMask[:, None] * cell_dmin[:, None] * \
                (term1 + lambda_S * -normal_array * curvVals[:, None])
           ...
           # print intensity_array[sample_pnt]
            ...
            if smooth_iter > 0:
                ...
                points_presmooth = vtk_to_numpy(
                    thisSurfVTK.GetPoints().GetData())
                thisSurfVTK = smooth_mesh(
                    thisSurfVTK, n_iter=smooth_iter, convergence=smooth_converge, relax=smooth_relax)

                points_postsmooth = vtk_to_numpy(
                    thisSurfVTK.GetPoints().GetData())
                ...
                # Restrict smoothing based on self proximity
                points_postsmooth[:, :] = points_presmooth + \
                    (points_postsmooth-points_presmooth)*cell_dmin[:, None]
            else:
                points_postsmooth = vtk_to_numpy(
                    thisSurfVTK.GetPoints().GetData())
            ...
            if output_itr_surface:
                if i % output_every_itr == 0:
                    if multisurf:
                        write_surf_vtk(
                            oldSurfVTK, procDir + "wm_itr" + str(i) + '_surf_' + str(m) + ".vtk")
                    else:
                        write_surf_vtk(oldSurfVTK, procDir + "wm_itr" + str(i)
                                       + '_fa' +
                                       str(target_val).replace('.', 'p')
                                       + '_LI' +
                                       str(lambda_I).replace('.', 'p')
                                       + '_LN' +
                                       str(lambda_N).replace('.', 'p')
...
def get_neighbors_ind(faces, vert_ind):
    ...
    face_ind = np.logical_or(np.logical_or(
        vert_ind == faces[:, 0], vert_ind == faces[:, 1]), vert_ind == faces[:, 2])
